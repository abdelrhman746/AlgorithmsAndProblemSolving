...
int ReadMonthInRange(string Message, int LowerRange, int HigherRange)
{
    int MonthOfYear;
    do
    {
        cout << Message << endl;
        cin >> MonthOfYear;
    } while (!(MonthOfYear >= LowerRange && MonthOfYear <= HigherRange));

    return MonthOfYear;
}

enYearMonth GetMonthOfYear(void)
{
    ...
    cout << "****************************************************************" << endl << endl;
   return (enYearMonth) ReadMonthInRange("Your choice is :", 1, 12);
...
void PrintMonthOfYear(enYearMonth MonthOfYear)
{
    cout << "Your Choice is ";
    switch (MonthOfYear)
    {
...
int main()
{
    PrintMonthOfYear(GetMonthOfYear());
}